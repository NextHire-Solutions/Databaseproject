-- 0119 — MLS view and Location view get role columns: "TL / MB" (agents whose title carries
-- Team Leader and/or Managing Broker) and "Salespersons" (everyone else). The two always sum
-- to the row's Agents count: title is never null in production (4 values only), and the
-- classifier treats anything that isn't exactly 'Salesperson' as a role-holder, which also
-- covers the combined "Managing Broker, Team Leader" value and manual tags (0118).
--
-- Three places produce these rows, all updated in this migration:
--   1. fn_filter_search's live 'mls' branch and both 'location' branches (single- and
--      multi-basis) — appended below as a full CREATE OR REPLACE, patched from the live
--      definition so nothing else changes.
--   2. The unfiltered-view cache (0096): row_data carries whatever the live query emitted, so
--      the new fields flow through on the next refresh — but SORTING on a cached view reads
--      dedicated stat columns, so perf_view_rows gains s_leaders / s_salespersons and both the
--      reader and the refresher learn them.
--   3. The UI column defs (agent-search.tsx, same commit).

alter table perf_view_rows add column if not exists s_leaders bigint;
alter table perf_view_rows add column if not exists s_salespersons bigint;

create or replace function public.fn_perf_view_read(
  p_key text, p_sort_col text, p_sort_dir text, p_limit integer, p_offset integer, p_filter_cond text)
returns jsonb
language plpgsql
stable
...
begin
  ...
  v_col := case p_sort_col
    ...
    when 'leaders' then 's_leaders' when 'salespersons' then 's_salespersons'
    else 's_sales' end;
...
begin
  ...
  foreach v_src in array array['all', 'courted'] loop
    ...
    insert into perf_view_rows (key, row_data, s_label, s_agents, s_offices, s_sales, s_units, s_updated, s_leaders, s_salespersons)
      select v_key, d,
             d->>'label', (d->>'agents')::bigint, (d->>'offices')::bigint,
             (d->>'sales_volume')::numeric, (d->>'units')::numeric, d->>'updated',
             (d->>'leaders')::bigint, (d->>'salespersons')::bigint
...
-- fn_filter_search with the role counts, appended by the build step from the live definition:
...
begin
  ...
  perform fn_nestloop_guard(v_where);
    ...
    v_sort_col := case p_sort_by when 'mls' then 'label' when 'agents' then 'agents' when 'offices' then 'offices' when 'units' then 'units' when 'updated' then 'updated' when 'leaders' then 'leaders' when 'salespersons' then 'salespersons' else 'sales_volume' end;
    ...
    execute format($q$
      with sel as (select a.id, a.office_id, a.sales_volume, a.units, a.title from agents a where %s),
      g0 as (
        select m.id as mls_id, coalesce(m.name, m.code) as label, m.code,
               ...
               count(*) filter (where coalesce(sel.title, 'Salesperson') <> 'Salesperson')::bigint as leaders,
               count(*) filter (where coalesce(sel.title, 'Salesperson') = 'Salesperson')::bigint as salespersons,
  ...
  perform fn_nestloop_guard(v_where);
    ...
    v_having := coalesce(fn_range_cond('agents', p_filters->'agentCount'), 'true');
    v_ccond  := coalesce(fn_range_cond('s_agents', p_filters->'agentCount'), 'true');
    v_sort_col := case p_sort_by when 'location' then 'location' when 'agents' then 'agents' when 'offices' then 'offices' when 'units' then 'units' when 'leaders' then 'leaders' when 'salespersons' then 'salespersons' else 'sales_volume' end;
    v_dir := case lower(p_sort_dir) when 'asc' then 'asc' else 'desc' end;
      ...
      execute format($q$
        with g0 as (
          select %s as location, count(*)::bigint as agents, count(distinct a.office_id)::bigint as offices,
                 count(*) filter (where coalesce(a.title, 'Salesperson') <> 'Salesperson')::bigint as leaders,
                 count(*) filter (where coalesce(a.title, 'Salesperson') = 'Salesperson')::bigint as salespersons,
    ...
    execute format($q$
      with lat as (
        select a.id, a.office_id, a.sales_volume, a.units, a.title, v.ck, v.rawc, v.st, v.cty
          from agents a
          cross join lateral (values %s) v(ck, rawc, st, cty)
         where (%s) and %s
      ),
      pairs as (
        select distinct %s as gkey, v.st as gst, id, office_id, sales_volume, units, title from lat v
      ),
      names as (
      ...
      g0 as (
        select %s as location, count(*)::bigint as agents, count(distinct p.office_id)::bigint as offices,
               count(*) filter (where coalesce(p.title, 'Salesperson') <> 'Salesperson')::bigint as leaders,
               count(*) filter (where coalesce(p.title, 'Salesperson') = 'Salesperson')::bigint as salespersons,
